...
final class FeaturesDTO extends Data implements BlockContract {
    ...
    public static function example(): View {
        return view('block_examples.features');
    }
...
<div class="bg-white py-24 sm:py-32">
  <div class="mx-auto max-w-7xl px-6 lg:px-8">
    <div class="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-3">
      <div>
        <h2 class="text-base font-semibold leading-7 text-indigo-600">{$this->subtitle}</h2>
        <p class="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{$this->title}</p>
        <p class="mt-6 text-base leading-7 text-gray-600">{$this->text}</p>
      </div>
